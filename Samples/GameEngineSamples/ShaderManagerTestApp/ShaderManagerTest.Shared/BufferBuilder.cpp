...
void BufferBuilder::BuildVertexBuffer(const std::string& name, size_t vertex_size, const byte_buffer& data_buffer)
{
    ...
    CommandBus::Post(ICommandPtr{ menew CreateVertexBuffer{m_vtxBufferName, (unsigned int)m_vtxSize, (unsigned int)m_vtxDataBuffer.size()} });
}
...
void BufferBuilder::BuildIndexBuffer(const std::string& name, const uint_buffer& data_buffer)
{
	...
	CommandBus::Post(ICommandPtr{ menew CreateIndexBuffer{m_idxBufferName, (unsigned int)(m_idxDataBuffer.size() * sizeof(unsigned int))} });
}
