// PackageContentTest.WinConsole.cpp : 此檔案包含 'main' 函式。程式會於該處開始執行及結束執行。
//
...
int main()
{
    ...
    stdfs::path path = stdfs::current_path();
    std::cout << "current path " << path << std::endl;
    stdfs::path media_path = path.parent_path().parent_path().parent_path() / "Media";
    std::cout << "media path " << media_path << std::endl;

    std::shared_ptr<AssetPackageFile> asset_pack = std::shared_ptr<AssetPackageFile>(
        AssetPackageFile::OpenPackage((media_path / "bundle").string()));
    if (!asset_pack)
    {
        std::cout << "open package error" << std::endl;
        return -1;
    }
    std::cout << "open package ok" << std::endl;

    FileSystem::Instance()->AddMountPath(IMountPathPtr{ menew PackageMountPath{ asset_pack, TEST_PATH_NAME } });
    ...
    for (int i = 0; i < FILE_MAX; i++)
    {
        ...
        futureFile[i] = FileSystem::Instance()->AsyncOpenFile(s_filename[i % 10], "rb", TEST_PATH_NAME);
    }
    ...
    while (ready_files < FILE_MAX)
    {
        for (int i = 0; i < FILE_MAX; i++)
        {
            ...
            else
            {
                ...
                std::cout << "File " << i << " size " << fileSize[i] << " Ready.\n";
                ready_files++;
            }
        }
    }
