...
void PortalSystemTest::makeSceneGraph()
{
    ...
    {
        const auto board_pawn_dto = SceneGraphMaker::makePawn(board_pawn_id, board_mesh_id, PrimitiveMeshMaker::getBoardBound());
    ...
    std::shared_ptr<HydratedPortalZoneNodeAssembler> inside_zone_assembler;
    std::shared_ptr<HydratedOutRegionNodeAssembler> outside_region_assembler;
    if (!m_sceneGraphFileStoreMapper->hasLaziedContent(outside_region_id))
    {
        outside_region_assembler = SceneGraphMaker::makeHydratedOutsideRegion(outside_region_id, m_rootId, portal_id, inside_zone_node_id, { floor_pawn_id, door_pawn_id });
        m_sceneGraphFileStoreMapper->putLaziedContent(outside_region_id, outside_region_assembler->assemble());
    }
    if (!m_sceneGraphFileStoreMapper->hasLaziedContent(inside_zone_node_id))
    {
        inside_zone_assembler = SceneGraphMaker::makeHydratedInsideZoneNode(inside_zone_node_id, portal_id, { board_pawn_id });
        m_sceneGraphFileStoreMapper->putLaziedContent(inside_zone_node_id, inside_zone_assembler->assemble());
    }
    ...
    if (!m_sceneGraph->root())
    {
        ...
        auto root = std::make_shared<RequestSpatialConstitution>(m_rootId, root_dto)->dispatch();
