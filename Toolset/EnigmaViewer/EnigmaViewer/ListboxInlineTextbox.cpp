    ...
    m_txt.events().key_char([this](const nana::arg_keyboard& arg)
        {
            if (arg.key == nana::keyboard::enter)
            {
                ...
                Enigma::Frameworks::EventPublisher::enqueue(std::make_shared<AnimationClipItemUpdated>(static_cast<unsigned int>(m_pos.cat), static_cast<unsigned int>(m_pos.item), m_prevText, m_txt.caption()));
