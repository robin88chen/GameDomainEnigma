...
#define AASSET_PATH_NAME "APK_PATH"
...
Java_com_AAssetTestApp_AAssetTestApp_initBridgeCallback( JNIEnv* env, jobject thiz )
{
...
extern "C"
JNIEXPORT void JNICALL
Java_com_AAssetTestApp_AAssetTestApp_initAdditionalMountPath(JNIEnv * env, jobject thiz)
{
    ...
    std::string path = AndroidBridge::GetDataFilePath();
    Debug::Printf("Data File Path %s", path.c_str());
    char u8s[] = u8"Testing 測試";
    char ss[1024];
    sprintf(ss, u8"Testing 測試");
    Debug::Printf(ss);
    Debug::Printf("Test Message End");
    if (FileSystem::Instance())
    {
        FileSystem::Instance()->AddMountPath
        (IMountPathPtr{ new AndroidMountPath("TestAssets/", AASSET_PATH_NAME) });
    }
}

extern "C"
JNIEXPORT jstring JNICALL
Java_com_AAssetTestApp_AAssetTestApp_getDataFilePath(JNIEnv * env, jobject thiz)
{
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_AAssetTestApp_AAssetTestApp_startStdioTest(JNIEnv * env, jobject thiz)
{
    //FileSystem::Create();
    ...
    for (int i = 0; i < FILE_MAX; i++)
    {
        ...
        futureFile[i] = FileSystem::Instance()->AsyncOpenFile(s_filename[i % 10], "rb", AASSET_PATH_NAME);
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_AAssetTestApp_AAssetTestApp_stopStdioTest(JNIEnv * env, jobject thiz)
{
    for (int i = 0; i < FILE_MAX; i++)
    {
        read_buf[i].clear();
