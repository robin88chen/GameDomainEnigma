...
std::shared_ptr<MeshPrimitive> SkinMeshModelMaker::makeCubeMeshPrimitive(const Enigma::Primitives::PrimitiveId& mesh_id, const Enigma::Geometries::GeometryId& geo_id)
{
    ...
    SkinMeshPrimitiveHelper mesh_helper(mesh_id);
    return mesh_helper.geometryId(geo_id).asNative(mesh_id.name() + ".mesh@DataPath").effect(EffectMaterialId("skin_mesh_prim_test")).textureMap(EffectTextureMapDtoHelper().textureMapping(TextureId("earth"), std::nullopt, "DiffuseMap")).renderListID(Enigma::Renderer::Renderer::RenderListID::Scene).visualTechnique("Default").constitute(PersistenceLevel::Store);
}
...
std::shared_ptr<ModelPrimitive> SkinMeshModelMaker::makeModelPrimitive(const Enigma::Primitives::PrimitiveId& model_id, const Enigma::Primitives::PrimitiveId& mesh_id, const Enigma::Animators::AnimatorId& animator_id, const std::vector<std::string>& mesh_node_names)
{
    ...
    MeshNodeTreeDtoHelper tree;
    for (unsigned i = 0; i < mesh_node_names.size(); i++)
    {
        MeshNodeDtoHelper node(mesh_node_names[i]);
    ...
    ModelPrimitiveHelper model_helper(model_id);
    return model_helper.asNative(model_id.name() + ".model@DataPath").animator(animator_id).meshNodeTree(tree).constitute(PersistenceLevel::Store);
}
