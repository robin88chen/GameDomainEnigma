                                                              ...
                                                              IntersectorCache* cache) const
{
    ...
    if (!mesh) return { false, cache };
    return TestMesh(mesh, ray, cache);
}
    ...
    const Ray3& point_ray, IntersectorCache* cache) const
{
    ...
    return intr_geo.Test(cache);
}

std::tuple<std::vector<IntrPrimitiveRay3::ResultRecord>, Intersector::Result> MeshPrimitiveRay3IntersectionFinder::FindMesh(
    const std::shared_ptr<MeshPrimitive>& mesh, const Ray3& point_ray, IntersectorCache* cache) const
{
    Ray3 ray = point_ray;
    Matrix4 inv_world = mesh->GetPrimitiveWorldTransform().Inverse();
    float dir_length;
