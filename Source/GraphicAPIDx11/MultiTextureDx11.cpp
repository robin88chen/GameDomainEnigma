...
error MultiTextureDx11::saveTextureImages(const std::vector<FileSystem::IFilePtr>& files)
{
    ...
    Frameworks::EventPublisher::enqueue(std::make_shared<Graphics::MultiTextureResourceImagesSaved>(m_name, filenames));
    return ErrorCode::ok;
}
...
error MultiTextureDx11::useAsBackSurface(const Graphics::IBackSurfacePtr& back_surf, const std::vector<Graphics::RenderTextureUsage>&)
{
    ...
    Frameworks::EventPublisher::enqueue(std::make_shared<Graphics::MultiTextureResourcesAsBackSurfaceUsed>(
