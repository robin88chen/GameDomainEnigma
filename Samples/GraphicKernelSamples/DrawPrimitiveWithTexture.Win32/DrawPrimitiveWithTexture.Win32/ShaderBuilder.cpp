                                      ...
                                      const std::string& shader_code, const std::string& layout_name)
{
	...
	CommandBus::Post(std::make_shared<CreateVertexShader>(m_vtxShaderName));
}
...
void ShaderBuilder::BuildPixelShader(const std::string& shader_name, const std::string& shader_code)
{
    ...
    CommandBus::Post(std::make_shared<CreatePixelShader>(m_pixShaderName));
}
...
void ShaderBuilder::OnVertexShaderCompiled(const IEventPtr& e)
{
    ...
    CommandBus::Post(std::make_shared<CreateVertexDeclaration>(m_vtxLayoutName, m_vtxFormatCode, shader.value()));
}
...
void ShaderBuilder::OnShaderBuilt(const IEventPtr& e)
{
    ...
    if ((m_hasVtxShaderBuilt) && (m_hasPixShaderBuilt))
    {
        ...
        IVertexDeclarationPtr vtx_layout = IGraphicAPI::Instance()->GetGraphicAsset<IVertexDeclarationPtr>(m_vtxLayoutName);
        if ((!vtx_shader) || (!pix_shader))
        {
            Debug::Printf("can't get vertex or pixel shader asset to create shader program");
            return;
        }
        CommandBus::Post(std::make_shared<CreateShaderProgram>(m_programName, vtx_shader, pix_shader, vtx_layout));
    }
}
