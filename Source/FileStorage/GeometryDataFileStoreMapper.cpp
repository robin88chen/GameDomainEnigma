...
GeometryDataFileStoreMapper::GeometryDataFileStoreMapper(const std::string& mapper_filename, const std::shared_ptr<Gateways::IDtoGateway>& gateway) : Engine::GeometryDataStoreMapper()
{
...
bool GeometryDataFileStoreMapper::hasGeometry(const Engine::GeometryId& id)
{
...
std::optional<Enigma::Engine::GenericDto> GeometryDataFileStoreMapper::queryGeometry(const Engine::GeometryId& id)
{
...
std::error_code GeometryDataFileStoreMapper::removeGeometry(const Engine::GeometryId& id)
{
    std::lock_guard locker{ m_fileMapLock };
...
std::error_code GeometryDataFileStoreMapper::putGeometry(const Engine::GeometryId& id, const Engine::GenericDto& dto)
{
...
void GeometryDataFileStoreMapper::deserializeMapperFile(const std::string& mapper_file_content)
{
    ...
    for (auto& line : lines)
    {
        ...
        Engine::GeometryId id{ tokens[0] };
        m_filename_map.insert_or_assign(id, tokens[1]);
    }
}

std::string GeometryDataFileStoreMapper::extractFilename(const Engine::GeometryId& id, const Engine::FactoryDesc& factory_desc)
{
